...
public class ConnectNeo4j
{
    ...
    //Retorna os ids dos autores de determinada instituição;
    public ArrayList<Integer> returnAuthorsByInstitution(String institution) {
    	...
    	try (Session session = driver.session())
        {
            ...
            StatementResult result = session.run(
            		"MATCH (a:Author)-[:ASSOCIATED_TO]->(i:Institution) WHERE i.name= {x} and a.name <> {x} RETURN ID(a) as id_author;",
                    parameters("x", institution));
            ...
            while(result.hasNext()){
                     ...
                     int author_id = record.get("id_author").asInt();
                     AuthorIDList.add(author_id);
    ...
    //Retorna a quantidade de nodos que existe no BD
    public int numberOfAuthors() {
    	...
    	try (Session session = driver.session())
        {
            ...
            if(result.hasNext()){
                     ...
                     Record record = result.next();
                     int quantidade = record.get("count").asInt();
    ...
    //Retorna a quantidade de relacionamentos de Coautorias
    public int numberOfCoAuthorRelationship() {
    ...
    public void addAlias(String name, Integer id_author){
        ...
        try (Session session = driver.session())
        {
            ...
            try (Transaction tx = session.beginTransaction())
            {
                	...
                	tx.run("MERGE (a:Alias {name: {x}})", parameters("x", name.trim()));
                	tx.run("MATCH (a:Alias {name: {x}}) MATCH (b:Author) WHERE ID(b) = " + id_author + " MERGE (a)-[:ALIAS]->(b)", parameters("x", name.trim()));
                	tx.success();
    	...
    	// Sessions are lightweight and disposable connection wrappers.
    		try (Session session = driver.session())
    		{
    			...
    			try (Transaction tx = session.beginTransaction())
    			{
    				...
    				//Se o autor tem um nome com acento, então já cria um Alias com o nome sem acento
    				if(!StringUtils.stripAccents(lista_autores.get(i).toString()).equalsIgnoreCase(lista_autores.get(i).toString())) {
    				
    				
    					// Já cria um Alias para cada autor com o nome sem acento
    					tx.run("MERGE (l:Alias {name: {x}})", parameters("x",  StringUtils.stripAccents(lista_autores.get(i).toString())));

    				
    					tx.run("MATCH (l:Alias {name: '" + StringUtils.stripAccents(lista_autores.get(i).toString()) + "'}) MATCH (a:Author {name: '" + lista_autores.get(i).toString() + "'})  MERGE (l)-[:ALIAS]->(a)");
